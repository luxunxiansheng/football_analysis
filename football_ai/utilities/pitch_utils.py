...
# Use simple tuples for colors (BGR format for OpenCV)
GREEN = (34, 139, 34)
WHITE = (255, 255, 255)
RED = (0, 0, 255)
BLACK = (0, 0, 0)
...
def draw_pitch(
    config: PitchConfig,
    background_color: Tuple[int, int, int] = GREEN,
...
def draw_points_on_pitch(
    config: PitchConfig,
    xy: np.ndarray,
    face_color: Tuple[int, int, int] = RED,
...
def draw_pitch_voronoi_diagram(
    ...
    team_1_color: Tuple[int, int, int] = RED,
    team_2_color: Tuple[int, int, int] = WHITE,
    opacity: float = 0.5,
    padding: int = 50,
    scale: float = 0.1,
    pitch: Optional[np.ndarray] = None,
    ...
    team_1_color_bgr = np.array(team_1_color, dtype=np.uint8)
    team_2_color_bgr = np.array(team_2_color, dtype=np.uint8)
